# Cuba Clasificados Bot

Telegram Mini App — локальная доска объявлений для Кубы (Аренда + Личные вещи + Услуги)

## Описание

Платформа для размещения и поиска объявлений внутри Telegram. Работает полностью внутри Telegram, без регистрации, оптимизирована под медленный интернет.

## Технологии

- **Backend**: Node.js + Express + PostgreSQL
- **Frontend**: React + Vite + Telegram Web App SDK
- **Bot**: node-telegram-bot-api

## Установка

### 1. Установка зависимостей

```bash
# Установка зависимостей backend и frontend
npm run install-all
```

### 2. Настройка базы данных

Создайте базу данных PostgreSQL:

```bash
createdb cuba_clasificados
```

Или используйте существующую базу данных и обновите `.env` файл.

### 3. Настройка переменных окружения

Создайте файл `.env` в корне проекта (используйте `.env.example` как шаблон):

```env
TELEGRAM_BOT_TOKEN=your_bot_token_here
DB_HOST=localhost
DB_PORT=5432
DB_NAME=cuba_clasificados
DB_USER=postgres
DB_PASSWORD=postgres
PORT=3000
NODE_ENV=development
FRONTEND_URL=http://localhost:5173
UPLOAD_DIR=./uploads
WEB_APP_URL=https://your-domain.com
```

### 4. Инициализация базы данных

База данных инициализируется автоматически при первом запуске сервера.

Или вручную:

```bash
psql -U postgres -d cuba_clasificados -f server/database/schema.sql
```

### 5. Запуск

#### Разработка

```bash
# Terminal 1: Backend
npm run dev

# Terminal 2: Frontend
cd frontend
npm run dev

# Terminal 3: Telegram Bot (опционально)
node bot/index.js
```

#### Production

```bash
# Сборка frontend
npm run build

# Запуск сервера
npm start
```

## Настройка Telegram Bot

1. Создайте бота через [@BotFather](https://t.me/BotFather)
2. Получите токен и добавьте его в `.env`
3. Настройте Web App через BotFather:
   ```
   /newapp
   Выберите вашего бота
   Название: Cuba Clasificados
   Описание: Доска объявлений для Кубы
   Фото: (опционально)
   Web App URL: https://your-domain.com
   ```

## Структура проекта

```
CubaClasificadosBot/
├── server/              # Backend
│   ├── database/        # Схема БД и инициализация
│   ├── middleware/      # Middleware (auth)
│   ├── routes/          # API routes
│   └── index.js         # Главный файл сервера
├── bot/                 # Telegram Bot
│   └── index.js         # Логика бота
├── frontend/            # Frontend (React)
│   ├── src/
│   │   ├── components/  # React компоненты
│   │   ├── App.jsx      # Главный компонент
│   │   └── main.jsx     # Точка входа
│   └── package.json
├── uploads/             # Загруженные изображения
└── package.json         # Backend зависимости
```

## API Endpoints

### Объявления

- `GET /api/listings` - Получить список объявлений
- `GET /api/listings/:id` - Получить объявление по ID
- `POST /api/listings` - Создать объявление
- `PATCH /api/listings/:id/status` - Обновить статус объявления
- `POST /api/listings/:id/promote` - Продвинуть объявление

### Города

- `GET /api/cities` - Получить список городов и районов

## Функциональность

### Категории

- 🏠 **Аренда** - Комнаты, квартиры, дома (NEIGHBORHOOD/CITY)
- 👕 **Личные вещи** - Одежда, электроника, мебель (COUNTRY/CITY/NEIGHBORHOOD)
- 🛠 **Услуги** - Ремонт, уборка, перевозки (CITY/NEIGHBORHOOD)

### Уровни охвата

- 📍 **NEIGHBORHOOD** - Район
- 🏙 **CITY** - Город
- 🇨🇺 **COUNTRY** - Вся Куба

### Фильтры

- Поиск по тексту
- Фильтр по цене (от/до)
- Фильтр по району
- Фильтр по категории

## Монетизация (MVP)

- Поднять объявление
- Закреп в категории/районе
- VIP-объявление

В MVP статус продвижения выставляется вручную через админку.

## Развертывание

### Рекомендуемые платформы

- **Backend**: Railway, Render, Heroku
- **Database**: Supabase, Railway PostgreSQL, Neon
- **Frontend**: Vercel, Netlify
- **Storage**: Cloudinary, AWS S3 (для изображений)

### Пример развертывания на Railway

1. Подключите GitHub репозиторий
2. Добавьте PostgreSQL addon
3. Настройте переменные окружения
4. Установите build command: `npm install && npm run build`
5. Установите start command: `npm start`

## Лицензия

ISC




